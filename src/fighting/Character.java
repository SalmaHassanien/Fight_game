...
public class Character {
    ...
    /**
     * The character side's flag.<br> {@code true} if the character is P1, or
     * {@code false} if P2.
     */
    private boolean playerNumber;
    /**
     * These sources are used for different sound effects such as walking
     * landing and different moves and projectiles.
     */
    ...
    /**
     * The character's facing direction.<br> {@code true} if the character is
     * facing right, {@code false} otherwise.
     */
    private boolean front;

    /**
     * The flag whether this character can run a new motion with the motion's
     * command.<br> {@code true} if the character can run a new motion,
    ...
    /**
     * The flag whether the motion hits the opponent or not.<br> {@code true} if
     * the motion hits the opponent, {@code false} otherwise.
     */
    ...
    /**
     * シミュレータ内での処理かどうかのフラグ．<br> {@code true} if the process is executed in the
     * simulator, {@code false} otherwise.
     */
    ...
    /**
     * To keep the crouch sound from playing over and over again while
     * crouching.
     */
    ...
    /**
     * For Projectile attack sound movement and hit detections.size is 3 because
     * 1 player can only have 3 projectiles active at a time.
     */
     ...
     * @param playerNumber the character's side flag．{@code true} if the player
     * is P1, or {@code false} if P2.
     */
...
//				SoundManager.getInstance().play2(sourceid4,SoundManager.getInstance().getSoundEffect().get(Name),this.x,this.y,true);
     ...
     *
     * @param opponentCenterX the opponent character's hit box's center
     * x-coordinate
     */
     ...
     * motion's command.
     *
     * @param control the flag whether this character can run a new motion with
     * the motion's command. {@code true} if the character can run,
     * {@code false} otherwise.
     */
     ...
     * normal status.
     *
     * @param remainingFrame the number of frames that the character needs to
     * resume to its normal status you want to set
     */
     ...
     * executing in the simulator.
     *
     * @param inputCommands a list storing keys of the action that the character
     * will be executing in the simulator
     */
     ...
     * simulator.
     *
     * @param inputCommands a list storing up to 30 keys that the character
